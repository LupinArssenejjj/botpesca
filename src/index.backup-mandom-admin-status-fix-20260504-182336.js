...
function formatMandomCleanStatusV1() {
  const now = Date.now();
  const snapshots = trimMandomCleanTimelineV1(readMandomCleanTimelineV1(), now);
  ...
  return [
    ...
    `Janela de rewind: *${formatDurationCompact(MANDOM_CLEAN_WINDOW_MS_V1)}*`,
    ``,
    `Mais antigo:`,
...
async function handleMandomCleanStatusCommandV1(message) {
  await replySafe(message, formatMandomCleanStatusV1());
}
...
client.on("message", async (message) => {
  try {
    ...
    if (command === "!mandom-status" || command === "!mandom") {
      await handleMandomCleanStatusCommandV1(message);
      return;
    }
