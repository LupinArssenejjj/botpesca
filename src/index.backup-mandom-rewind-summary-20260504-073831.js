...
async function activateMandom(state, player) {
  ...
  await sendGroupMessage(
    [
      ...
      `> Pesca, itens, iscas, Stands, partes e eventos podem ser revertidos.`
  ...
  if (snapshot) {
    ...
    await sendGroupMessage(
      [
        ...
        ``,
        `> O usuário de Mandom manteve o Stand.`,
        `> Cooldown aplicado: *${formatDurationCompact(cooldownMs)}*`,
  ...
  await sendGroupMessage(
    [
      ...
      ``,
      `> Capturas apagadas: *${fallback.removedCount}*`,
      `> Iscas restauradas: *${fallback.refundedCount}*`,
      `> Cooldown aplicado: *${formatDurationCompact(cooldownMs)}*`
    ].join(mandomTimelineNlV1())
  );
