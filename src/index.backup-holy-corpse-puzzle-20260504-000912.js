...
async function handleHolyCorpseUseCommand(message, state, player, arg) {
  ...
  if (part.key === "left_arm") {
    if (!STAND_DEFS.tusk_act_1) {
      await replySafe(message, `💪 *Tusk Act 1* ainda não está instalado no sistema.`);
      return;
    }
    ...
    if (player.stand) {
      await replySafe(
        ...
        [
          `💪 *Left Arm reagiu...*`,
          ``,
          `Mas você já possui um Stand.`,
          `> Guarde ou remova o Stand atual antes de despertar *Tusk Act 1*.`
        ].join(cadaverNl())
      );
      return;
    }

    consumeHolyCorpsePart(player, "left_arm");
    player.stand = { key: "tusk_act_1" };
    player.standCooldownUntil = 0;
    player.activeStandBuff = null;
    ...
    await sendGroupMessage(
      [
        `💪 *A Left Arm do Cadáver Santo reagiu!*`,
        ``,
        `*${player.name}* despertou *💅 Tusk Act 1*.`,
        `> A rotação começou a se mover.`
      ].join(cadaverNl())
    );
    return;
  }
  ...
  if (part.key === "spine") {
    if (!STAND_DEFS.mandom) {
      await replySafe(message, `🦴 *Mandom* ainda não está disponível no sistema.`);
      return;
    }
    ...
    if (player.stand) {
      await replySafe(
        ...
        [
          ...
          ``,
          `Mas você já possui um Stand.`,
          `> Guarde ou remova o Stand atual antes de despertar *Mandom*.`
        ].join(cadaverNl())
      );
      return;
    }

    consumeHolyCorpsePart(player, "spine");
    player.stand = { key: "mandom" };
    player.standCooldownUntil = 0;
    player.activeStandBuff = null;
    ...
    await sendGroupMessage(
      [
        `🦴 *A Spine do Cadáver Santo reagiu!*`,
        ``,
        `*${player.name}* despertou *Mandom*.`,
        `> O tempo começou a voltar sobre si mesmo.`
      ].join(cadaverNl())
    );
    return;
  }
  ...
  await replySafe(
    ...
    [
      `${part.emoji} *${part.name}*`,
      ``,
      getHolyCorpseUsePreview(part.key) || `Essa parte ainda não possui uso configurado.`,
      ``,
      `> Essa parte foi mantida no inventário.`
    ].join(cadaverNl())
  );
}
...
function ensureHolyCorpseEventStateV1(state) {
  ...
  if (!event.lastParticipants || !Array.isArray(event.lastParticipants)) {
...
async function closeHolyCorpseApproachWindowV1() {
  ...
  event.phase = "quest_pending";
  event.closedAt = Date.now();
  event.lastParticipants = participants.map((participant) => ({
    id: participant.id,
    name: participant.name,
    approachedAt: participant.approachedAt
  }));
  ...
  await sendGroupMessage(
    [
      `🧩 *A quest do Cadáver Santo foi formada!*`,
      ``,
      ...
      ``,
      `> O sistema de puzzle será ativado no próximo patch.`,
      `> Por enquanto, o spawn e o !aproximar já estão funcionando.`
    ].join(holyCorpseSpawnNlV1())
  );
}
