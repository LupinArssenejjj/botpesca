...
// PRIVATE_ADMIN_ROOT_V1_START

const PRIVATE_ADMIN_ROOT_PHONE_V1 = "24998805233";
const PRIVATE_ADMIN_ROOT_COMMANDS_V1 = new Set(["!admin", "!adm", "!root"]);

function adminRootNlV1() {
  return String.fromCharCode(10);
}

function adminRootDigitsV1(value) {
  return String(value || "").replace(/\D/g, "");
}

function adminRootNormalizeTextV1(value) {
  return String(value || "")
    .trim()
    .toLowerCase()
    .normalize("NFD")
...
function adminRootIsOwnerV1(message) {
  const ownerDigits = adminRootDigitsV1(PRIVATE_ADMIN_ROOT_PHONE_V1);
  ...
  return possibleIds.some((value) => {
    const digits = adminRootDigitsV1(value);

    return Boolean(ownerDigits) && Boolean(digits) && (
      digits === ownerDigits ||
      digits.endsWith(ownerDigits) ||
      digits.endsWith(`55${ownerDigits}`)
    );
  });
}

function adminRootShortIdV1(id) {
  const digits = adminRootDigitsV1(id);
  if (!digits) return String(id || "");
  return digits.length > 6 ? `...${digits.slice(-6)}` : digits;
}

async function adminRootReplyV1(message, text) {
  try {
    await message.reply(text);
  } catch (error) {
    log("Erro ao responder admin privado:", error.message);
  }
}

function adminRootMenuV1() {
  return [
    `🗝️ *ROOT PRIVADO — Bot da Pescaria*`,
    ``,
    `Acesso aceito somente no privado do dono.`,
    `Nada daqui envia mensagem no grupo.`,
    ``,
    `📋 *Consulta*`,
    `• !admin status`,
    `• !admin jogadores`,
    `• !admin ver <nome|telefone|id>`,
    ``,
    `🎣 *Pescaria / ranking*`,
    `• !admin iscas <alvo> <qtd>`,
    `• !admin add-iscas <alvo> <qtd>`,
    `• !admin peixe <alvo> <kg> [nome]`,
    `• !admin limpar-peixes-admin <alvo>`,
    ``,
    `🧬 *Stand / itens*`,
    `• !admin stand <alvo> <stand_key|none>`,
    `• !admin item <alvo> rokakaka <qtd>`,
    `• !admin item <alvo> disco-vazio <qtd>`,
    `• !admin disco-stand <alvo> <stand_key> [qtd]`,
    `• !admin cadaver <alvo> <parte> [qtd]`,
    ``,
    `🍆 *Pinto*`,
    `• !admin pinto-score <alvo> <score>`,
    `• !admin pinto-reset <alvo>`,
    ``,
    `🧹 *Controle*`,
    `• !admin reset-cooldowns <alvo>`,
    `• !admin banir <nome|telefone|id>`,
    `• !admin backup`,
    ``,
    `Exemplos:`,
    `• !admin ver D.`,
    `• !admin iscas D. 10`,
    `• !admin stand D. tw_au`,
    `• !admin cadaver D. left_arm 1`,
    `• !admin banir 5524999999999`
  ].join(adminRootNlV1());
}

function adminRootStatusV1(state) {
  const totalPlayers = Object.keys(state.players || {}).length;
  const totalFish = Number(state.groupStats?.totalFish || 0);
  const totalTrash = Number(state.groupStats?.totalTrash || 0);
  const totalLegendary = Number(state.groupStats?.totalLegendary || 0);
  const groupId = ALLOWED_GROUP_ID || "não configurado";

  return [
    `🗝️ *Status ROOT*`,
    ``,
    `Grupo permitido: ${groupId}`,
    `Jogadores no estado: ${totalPlayers}`,
    `Peixes totais: ${totalFish}`,
    `Lixos totais: ${totalTrash}`,
    `Lendas totais: ${totalLegendary}`,
    `Arquivo: data/pesca.json`,
    ``,
    `Dono privado: ${PRIVATE_ADMIN_ROOT_PHONE_V1}`
  ].join(adminRootNlV1());
}

function adminRootPlayerRowsV1(state) {
  return Object.values(state.players || {})
    .sort((a, b) => {
      const scoreA = Number(a.totalWeight || 0) + Number(a.totalFish || 0);
      const scoreB = Number(b.totalWeight || 0) + Number(b.totalFish || 0);
      return scoreB - scoreA;
    })
    .map((player, index) => {
      const stand = player.stand?.key && STAND_DEFS[player.stand.key]
        ? `${STAND_DEFS[player.stand.key].emoji} ${STAND_DEFS[player.stand.key].name}`
        : "sem Stand";
      return `${index + 1}. ${player.name} — ${adminRootShortIdV1(player.id)} — ${formatWeight(player.totalWeight || 0)} — ${player.baits}/${getMaxBaits(player)} iscas — ${stand}`;
    });
}

function adminRootListPlayersV1(state) {
  const rows = adminRootPlayerRowsV1(state);

  return [
    `👥 *Jogadores cadastrados*`,
    ``,
    rows.length ? rows.join(adminRootNlV1()) : `_Nenhum jogador cadastrado ainda._`
  ].join(adminRootNlV1());
}

function adminRootResolvePlayerV1(state, rawQuery) {
  const query = String(rawQuery || "").trim();
  const queryDigits = adminRootDigitsV1(query);
  const queryText = adminRootNormalizeTextV1(query);

  if (!query) {
    return { error: "Informe um alvo." };
  }
  ...
  const exact = players.find((player) => {
    ...
    const playerDigits = adminRootDigitsV1(id);
    const playerName = adminRootNormalizeTextV1(player.name);

    return id === query ||
      playerDigits === queryDigits ||
      (queryDigits.length >= 6 && playerDigits.endsWith(queryDigits)) ||
      playerName === queryText;
  });
  ...
  const partial = players.filter((player) => {
    ...
    return (queryDigits.length >= 4 && playerDigits.includes(queryDigits)) ||
      (queryText.length >= 2 && playerName.includes(queryText));
  });

  if (partial.length === 1) {
    return { player: partial[0] };
  }
  ...
  if (partial.length > 1) {
    return {
      error: [
        `Alvo ambíguo: ${query}`,
        ``,
        ...partial.slice(0, 12).map((player) => `• ${player.name} — ${adminRootShortIdV1(player.id)}`)
      ].join(adminRootNlV1())
    };
  }

  return { error: `Não achei jogador para: ${query}` };
}

function adminRootStandNameV1(player) {
  if (!player.stand?.key || !STAND_DEFS[player.stand.key]) return "Nenhum";
  const stand = STAND_DEFS[player.stand.key];
  return `${stand.emoji} ${stand.name} (${stand.key})`;
}

function adminRootFormatPlayerV1(state, player) {
  ensureSpecialItems(player);
  if (typeof ensureHolyCorpseInventory === "function") {
    ensureHolyCorpseInventory(player);
  }

  const pinto = state.miniGames?.pinto?.players?.[player.id] || null;
  const parts = player.holyCorpse?.parts && typeof HOLY_CORPSE_PARTS === "object"
    ? Object.values(HOLY_CORPSE_PARTS)
      .map((part) => `${part.emoji} ${part.key}: ${Number(player.holyCorpse.parts[part.key] || 0)}`)
      .join(" | ")
    : "sem inventário";
  ...
  return [
    ...
    ``,
    `ID: ${player.id}`,
    `Iscas: ${player.baits}/${getMaxBaits(player)}`,
    ...
    `Maior peixe: ${player.biggestCatch ? `${player.biggestCatch.emoji} ${player.biggestCatch.name} — ${formatWeight(player.biggestCatch.weightKg)}` : "nenhum"}`,
    ``,
    `Stand: ${adminRootStandNameV1(player)}`,
    `Cooldown Stand até: ${player.standCooldownUntil ? new Date(player.standCooldownUntil).toLocaleString("pt-BR") : "liberado"}`,
    ``,
    `Rokakaka: ${player.specialItems.rokakaka}`,
    `Discos vazios: ${player.specialItems.blankStandDiscs}`,
    `Discos com Stand: ${discs || "nenhum"}`,
    ``,
    `Cadáver Santo: ${parts}`,
    ``,
    `Pinto score: ${pinto ? Number(pinto.score || 0) : "sem registro"}`
  ].join(adminRootNlV1());
}

function adminRootCommitStateV1(state) {
  normalizeAllPlayers(state);
...
function adminRootParseAmountV1(value, fallback = null) {
...
function adminRootClampIntV1(value, min, max) {
  const parsed = Math.trunc(Number(value));
  if (!Number.isFinite(parsed)) return min;
  return Math.max(min, Math.min(max, parsed));
}

function adminRootSetBaitsV1(state, args, additive) {
  const [targetRaw, amountRaw] = args;
  const amount = adminRootParseAmountV1(amountRaw, null);

  if (!targetRaw || amount === null) {
    return `Uso: !admin ${additive ? "add-iscas" : "iscas"} <alvo> <qtd>`;
  }

  const resolved = adminRootResolvePlayerV1(state, targetRaw);
  if (!resolved.player) return resolved.error;

  const player = resolved.player;
  const before = Number(player.baits || 0);

  if (additive) {
    player.baits = adminRootClampIntV1(before + amount, 0, 999);
  } else {
    player.baits = adminRootClampIntV1(amount, 0, 999);
  }

  player.lastBaitAt = Date.now();
  adminRootCommitStateV1(state);

  return [
    `✅ *Iscas alteradas*`,
    ``,
    `${player.name}: ${before} → ${player.baits}`
  ].join(adminRootNlV1());
}

function adminRootAddAdminFishV1(state, args) {
  const [targetRaw, kgRaw, ...nameParts] = args;
  const kg = adminRootParseAmountV1(kgRaw, null);

  if (!targetRaw || kg === null || kg <= 0) {
    return `Uso: !admin peixe <alvo> <kg> [nome]`;
  }
  ...
  const player = resolved.player;
  const fishName = nameParts.join(" ").trim() || "Peixe Administrativo";
  ...
  return [
    ...
    `${player.name}`,
    `🐟 ${fishName}`,
    `Peso: ${formatWeight(kg)}`,
    `Novo total: ${formatWeight(player.totalWeight || 0)}`
  ].join(adminRootNlV1());
}

function adminRootClearAdminFishV1(state, args) {
  const [targetRaw] = args;

  if (!targetRaw) {
    return `Uso: !admin limpar-peixes-admin <alvo>`;
  }
  ...
  player.history = player.history.filter((item) => item.source !== "admin_score");
  const removed = before - player.history.length;

  adminRootCommitStateV1(state);

  return [
    `✅ *Peixes administrativos removidos*`,
    ``,
    `${player.name}`,
    `Removidos: ${removed}`,
    `Peso atual: ${formatWeight(player.totalWeight || 0)}`
  ].join(adminRootNlV1());
}

function adminRootSetStandV1(state, args) {
  const [targetRaw, standRaw] = args;

  if (!targetRaw || !standRaw) {
    return `Uso: !admin stand <alvo> <stand_key|none>`;
  }
  ...
  const player = resolved.player;
  const standKey = adminRootNormalizeTextV1(standRaw).replace(/[^a-z0-9_:-]/g, "_").replace(/:+/g, "_");

  if (["none", "nenhum", "remover", "null", "0"].includes(standKey)) {
    player.stand = null;
    player.standCooldownUntil = 0;
    player.activeStandBuff = null;
    adminRootCommitStateV1(state);
    ...
    return [
      `✅ *Stand removido*`,
      ``,
      `${player.name} agora está sem Stand.`
    ].join(adminRootNlV1());
  }

  if (!STAND_DEFS[standKey]) {
    const keys = Object.keys(STAND_DEFS).sort().join(", ");
    return [
      `⛔ Stand inválido: ${standRaw}`,
      ``,
      `Keys disponíveis:`,
      keys
    ].join(adminRootNlV1());
  ...
  player.stand = { key: standKey };
  player.standCooldownUntil = 0;
  player.activeStandBuff = null;
  adminRootCommitStateV1(state);

  const stand = STAND_DEFS[standKey];

  return [
    `✅ *Stand alterado*`,
    ``,
    `${player.name} recebeu ${stand.emoji} *${stand.name}*.`
  ].join(adminRootNlV1());
}

function adminRootChangeItemV1(state, args) {
  const [targetRaw, itemRaw, amountRaw] = args;
  const amount = adminRootParseAmountV1(amountRaw, null);

  if (!targetRaw || !itemRaw || amount === null) {
    return `Uso: !admin item <alvo> rokakaka|disco-vazio <qtd>`;
  }
  ...
  const item = adminRootNormalizeTextV1(itemRaw).replace(/_/g, "-");
  const safeAmount = Math.trunc(amount);

  if (item === "rokakaka" || item === "rokaka") {
    const before = player.specialItems.rokakaka;
    player.specialItems.rokakaka = Math.max(0, before + safeAmount);
    adminRootCommitStateV1(state);

    return `✅ Rokakaka de ${player.name}: ${before} → ${player.specialItems.rokakaka}`;
  }

  if (item === "disco-vazio" || item === "disco" || item === "blank-disc") {
    const before = player.specialItems.blankStandDiscs;
    player.specialItems.blankStandDiscs = Math.max(0, before + safeAmount);
    adminRootCommitStateV1(state);

    return `✅ Discos vazios de ${player.name}: ${before} → ${player.specialItems.blankStandDiscs}`;
  }

  return `⛔ Item inválido. Use: rokakaka ou disco-vazio.`;
}

function adminRootAddStandDiscV1(state, args) {
  const [targetRaw, standRaw, amountRaw] = args;
  const amount = adminRootClampIntV1(adminRootParseAmountV1(amountRaw, 1), 1, 20);

  if (!targetRaw || !standRaw) {
    return `Uso: !admin disco-stand <alvo> <stand_key> [qtd]`;
  }
  ...
  if (!STAND_DEFS[standKey]) {
    return `⛔ Stand inválido: ${standRaw}`;
  }
  ...
  return [
    ...
    ``,
    `${player.name}`,
    `${amount}x 💿 ${stand.emoji} ${stand.name}`
  ].join(adminRootNlV1());
}

function adminRootChangeCadaverPartV1(state, args) {
  const [targetRaw, partRaw, amountRaw] = args;
  const amount = adminRootClampIntV1(adminRootParseAmountV1(amountRaw, 1), -99, 99);

  if (!targetRaw || !partRaw) {
    return `Uso: !admin cadaver <alvo> <parte> [qtd]`;
  }

  if (typeof normalizeHolyCorpsePartKey !== "function" || typeof ensureHolyCorpseInventory !== "function" || typeof HOLY_CORPSE_PARTS !== "object") {
    return `⛔ Sistema do Cadáver Santo não está disponível neste arquivo.`;
  }
  ...
  const partKey = normalizeHolyCorpsePartKey(partRaw);
  const part = HOLY_CORPSE_PARTS[partKey];

  if (!part) {
    return `⛔ Parte inválida: ${partRaw}`;
  }

  const before = Number(player.holyCorpse.parts[part.key] || 0);
  const after = Math.max(0, before + amount);
  player.holyCorpse.parts[part.key] = after;
  ...
  return [
    ...
    ``,
    `${player.name}`,
    `${part.emoji} ${part.name}: ${before} → ${after}`
  ].join(adminRootNlV1());
}

function adminRootResetCooldownsV1(state, args) {
  const [targetRaw] = args;

  if (!targetRaw) {
    return `Uso: !admin reset-cooldowns <alvo>`;
  }
  ...
  player.standCooldownUntil = 0;
  player.activeStandBuff = null;
  player.lastBaitAt = Date.now() - 30 * 60 * 1000;

  ensureMiniGamesState(state);
  const pinto = state.miniGames.pinto.players[player.id];
  if (pinto) {
    pinto.lastPlayedAt = 0;
  }
...
function adminRootSetPintoScoreV1(state, args) {
  const [targetRaw, scoreRaw] = args;
  const score = adminRootParseAmountV1(scoreRaw, null);

  if (!targetRaw || score === null) {
    return `Uso: !admin pinto-score <alvo> <score>`;
  }
  ...
  const player = resolved.player;
  const pinto = getOrCreatePintoPlayer(state, player.id, player.name);
  const before = Number(pinto.score || 0);

  pinto.score = Math.max(0, Math.trunc(score));
  pinto.lastResultAt = Date.now();

  saveState(state);

  return [
    `✅ *Score do pinto alterado*`,
    ``,
    `${player.name}: ${before} → ${pinto.score}`
  ].join(adminRootNlV1());
}

function adminRootResetPintoV1(state, args) {
  const [targetRaw] = args;

  if (!targetRaw) {
    return `Uso: !admin pinto-reset <alvo>`;
  }
  ...
  return [
    `✅ *Pinto resetado*`,
    ``,
    `${player.name} pode rodar !pinto de novo.`
  ].join(adminRootNlV1());
}

function adminRootBackupStateV1() {
  ensureDataFile();

  const stamp = new Date().toISOString().replace(/[:.]/g, "-");
  const backupPath = path.join(DATA_DIR, `pesca.admin-backup-${stamp}.json`);
...
async function adminRootFindParticipantIdV1(state, query) {
  const resolved = adminRootResolvePlayerV1(state, query);

  if (resolved.player) {
    return { id: resolved.player.id, label: resolved.player.name };
  }

  const queryDigits = adminRootDigitsV1(query);

  if (!queryDigits) {
    return { error: resolved.error || `Não achei participante: ${query}` };
  }

  if (!ALLOWED_GROUP_ID) {
    return { error: "ALLOWED_GROUP_ID não configurado." };
  }
  ...
  const match = participants.find((participant) => {
    const id = participant?.id?._serialized || participant?.id?.user || "";
    const digits = adminRootDigitsV1(id);
    return digits === queryDigits ||
      digits.endsWith(queryDigits) ||
      digits.endsWith(`55${queryDigits}`);
  });

  if (match) {
    const id = match.id._serialized || `${match.id.user}@c.us`;
    return { id, label: adminRootShortIdV1(id) };
  }

  const withCountry = queryDigits.startsWith("55") ? `${queryDigits}@c.us` : `55${queryDigits}@c.us`;
  return { id: withCountry, label: adminRootShortIdV1(withCountry) };
}

async function adminRootBanV1(state, args) {
  const [targetRaw] = args;

  if (!targetRaw) {
    return `Uso: !admin banir <nome|telefone|id>`;
  }

  if (!ALLOWED_GROUP_ID) {
    return `⛔ ALLOWED_GROUP_ID não configurado.`;
  }

  const participant = await adminRootFindParticipantIdV1(state, targetRaw);
  if (!participant.id) return participant.error;

  const groupChat = await client.getChatById(ALLOWED_GROUP_ID);

  if (!groupChat || !groupChat.isGroup) {
    return `⛔ Não consegui abrir o grupo permitido.`;
  }

  if (typeof groupChat.removeParticipants !== "function") {
    return `⛔ Este whatsapp-web.js não expôs removeParticipants neste chat.`;
  }

  await groupChat.removeParticipants([participant.id]);

  return [
    `✅ *Remoção solicitada*`,
    ``,
    `Alvo: ${participant.label}`,
    `ID: ${participant.id}`,
    ``,
    `O bot não enviou mensagem no grupo. O WhatsApp ainda pode exibir evento nativo de remoção.`
  ].join(adminRootNlV1());
}

async function handlePrivateAdminRootV1(message, arg) {
  const [subRaw, ...args] = String(arg || "").trim().split(/\s+/).filter(Boolean);
  const sub = adminRootNormalizeTextV1(subRaw || "menu").replace(/_/g, "-");

  const state = loadState();
  normalizeAllPlayers(state);

  if (["menu", "help", "ajuda", ""].includes(sub)) {
    await adminRootReplyV1(message, adminRootMenuV1());
    return;
  }

  if (sub === "status") {
    await adminRootReplyV1(message, adminRootStatusV1(state));
    return;
  }

  if (sub === "jogadores" || sub === "players" || sub === "listar") {
    await adminRootReplyV1(message, adminRootListPlayersV1(state));
    return;
  }

  if (sub === "ver" || sub === "info") {
    const target = args[0];
    const resolved = adminRootResolvePlayerV1(state, target);
    await adminRootReplyV1(message, resolved.player ? adminRootFormatPlayerV1(state, resolved.player) : resolved.error);
    return;
  }

  if (sub === "iscas") {
    await adminRootReplyV1(message, adminRootSetBaitsV1(state, args, false));
    return;
  }

  if (sub === "add-iscas" || sub === "somar-iscas") {
    await adminRootReplyV1(message, adminRootSetBaitsV1(state, args, true));
    return;
  }

  if (sub === "peixe" || sub === "add-peixe" || sub === "pontuacao" || sub === "pontuação") {
    await adminRootReplyV1(message, adminRootAddAdminFishV1(state, args));
    return;
  }

  if (sub === "limpar-peixes-admin" || sub === "limpar-pontuacao-admin" || sub === "limpar-pontuação-admin") {
    await adminRootReplyV1(message, adminRootClearAdminFishV1(state, args));
    return;
  }

  if (sub === "stand") {
    await adminRootReplyV1(message, adminRootSetStandV1(state, args));
    return;
  }

  if (sub === "item" || sub === "itens") {
    await adminRootReplyV1(message, adminRootChangeItemV1(state, args));
    return;
  }

  if (sub === "disco-stand" || sub === "stand-disco") {
    await adminRootReplyV1(message, adminRootAddStandDiscV1(state, args));
    return;
  }

  if (sub === "cadaver" || sub === "cadáver" || sub === "parte") {
    await adminRootReplyV1(message, adminRootChangeCadaverPartV1(state, args));
    return;
  }

  if (sub === "reset-cooldowns" || sub === "reset-cooldown" || sub === "cooldowns") {
    await adminRootReplyV1(message, adminRootResetCooldownsV1(state, args));
    return;
  }

  if (sub === "pinto-score") {
    await adminRootReplyV1(message, adminRootSetPintoScoreV1(state, args));
    return;
  }

  if (sub === "pinto-reset") {
    await adminRootReplyV1(message, adminRootResetPintoV1(state, args));
    return;
  }

  if (sub === "backup") {
    await adminRootReplyV1(message, adminRootBackupStateV1());
    return;
  }

  if (sub === "banir" || sub === "ban" || sub === "remover") {
    await adminRootReplyV1(message, await adminRootBanV1(state, args));
    return;
  }

  await adminRootReplyV1(
    message,
    [
      `⛔ Comando admin desconhecido: ${subRaw}`,
      ``,
      `Use: !admin`
    ].join(adminRootNlV1())
  );
...
async function maybeHandlePrivateAdminRootV1(message, chat, rawBody) {
  if (!rawBody.startsWith(COMMAND_PREFIX)) {
    return false;
  }

  const parsed = parseCommand(rawBody);

  if (!PRIVATE_ADMIN_ROOT_COMMANDS_V1.has(parsed.command)) {
    return false;
  }
  ...
  if (!adminRootIsOwnerV1(message)) {
    return true;
  }

  try {
    await handlePrivateAdminRootV1(message, parsed.arg);
  } catch (error) {
    log("Erro no admin privado:", error.message);
    await adminRootReplyV1(
      message,
      [
        `💥 *Erro no admin privado*`,
        ``,
        error.message
      ].join(adminRootNlV1())
    );
...
client.on("message", async (message) => {
  try {
    ...
    if (await maybeHandlePrivateAdminRootV1(message, chat, rawBody)) return;
