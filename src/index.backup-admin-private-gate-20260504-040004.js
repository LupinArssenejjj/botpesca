...
function adminRootIsOwnerV1(message) {
  const fromDigits = adminRootDigitsV1(message?.from || message?.author || "");
  const ownerDigits = adminRootDigitsV1(PRIVATE_ADMIN_ROOT_PHONE_V1);

  return Boolean(ownerDigits) && (
    fromDigits === ownerDigits ||
    fromDigits.endsWith(ownerDigits) ||
    fromDigits.endsWith(`55${ownerDigits}`)
  );
}
