...
function getPintoStatusText(score) {
  if (score >= 900) return "💀 *ABSURDO.* Isso aqui já entra em discussão geopolítica.";
  if (score >= 750) return "🫡 *MONSTRUOSO.* O SUS não cobre esse tipo de risco.";
  if (score >= 600) return "😳 *MUITO FORTE.* Estatisticamente preocupante.";
  if (score >= 450) return "😐 *OK.* Cumpre o que promete, sem grandes firulas.";
  if (score >= 300) return "🙂 *HONESTO.* Trabalhador e esforçado.";
  if (score >= 180) return "🤏 *MODESTO.* Compacto, urbano e econômico.";
  return "🪦 *CRÍTICO.* Vamos orar pela autoestima do paciente.";
}

function generatePintoResult() {
  const flaccidCm = round(randomBetween(2.1, 14.8));
  const erectCm = round(randomBetween(7.2, 27.5));
  const girthCm = round(randomBetween(6.1, 18.2));
