...
const STAND_DEFS = {
  ...
  mandom: {
    ...
    rarity: "lendário",
    rarityScore: 6,
    passiveDescription: "Rebobinagem total: registra snapshots persistentes e pode voltar o estado inteiro do grupo em até 4 minutos.",
    activeName: "Rewind",
    activeDescription: "Rebobina o estado completo do grupo: pescas, iscas, itens, Stands, partes do Cadáver Santo e eventos.",
...
// MANDOM_TIMELINE_REWIND_V1_START

const MANDOM_TIMELINE_WINDOW_MS_V1 = 4 * 60 * 1000;
const MANDOM_TIMELINE_KEEP_EXTRA_MS_V1 = 30 * 1000;
const MANDOM_TIMELINE_MAX_SNAPSHOTS_V1 = 180;

let mandomTimelineSnapshotsV1 = [];
let mandomTimelineRestoringV1 = false;
let mandomTimelineLastActivationAtV1 = 0;

function mandomTimelineNowV1() {
  return Date.now();
}

function mandomTimelineNlV1() {
  return String.fromCharCode(10);
}

function mandomTimelineTrimV1(now = mandomTimelineNowV1()) {
  const minAt = Math.max(0, now - MANDOM_TIMELINE_WINDOW_MS_V1 - MANDOM_TIMELINE_KEEP_EXTRA_MS_V1);

  mandomTimelineSnapshotsV1 = mandomTimelineSnapshotsV1
    .filter((snapshot) => snapshot && Number(snapshot.at || 0) >= minAt && snapshot.raw)
    .slice(-MANDOM_TIMELINE_MAX_SNAPSHOTS_V1);
}

function mandomTimelineCaptureBeforeSaveV1() {
  if (mandomTimelineRestoringV1) {
    return;
  }
  ...
  try {
    ...
    const last = mandomTimelineSnapshotsV1[mandomTimelineSnapshotsV1.length - 1];

    if (last && last.raw === compactRaw) {
      last.at = mandomTimelineNowV1();
      mandomTimelineTrimV1();
      return;
    }

    mandomTimelineSnapshotsV1.push({
      at: mandomTimelineNowV1(),
      raw: compactRaw
    });

    mandomTimelineTrimV1();
  } catch (error) {
    log("Mandom timeline: não consegui capturar snapshot:", error.message);
  }
}

function mandomTimelinePickSnapshotV1(now = mandomTimelineNowV1()) {
  mandomTimelineTrimV1(now);

  if (!mandomTimelineSnapshotsV1.length) {
    return null;
  }

  const windowStart = Math.max(
    now - MANDOM_TIMELINE_WINDOW_MS_V1,
    Number(mandomTimelineLastActivationAtV1 || 0)
  );

  const insideWindow = mandomTimelineSnapshotsV1
    .filter((snapshot) => Number(snapshot.at || 0) >= windowStart && Number(snapshot.at || 0) <= now)
    .sort((a, b) => Number(a.at || 0) - Number(b.at || 0));

  if (insideWindow.length) {
    return insideWindow[0];
  }

  const beforeWindow = mandomTimelineSnapshotsV1
    .filter((snapshot) => Number(snapshot.at || 0) <= windowStart)
    .sort((a, b) => Number(a.at || 0) - Number(b.at || 0));

  if (beforeWindow.length) {
    return beforeWindow[beforeWindow.length - 1];
  }

  return mandomTimelineSnapshotsV1[0] || null;
}

function mandomTimelineClonePlayerIdentityV1(player) {
  return {
    id: player.id,
    name: player.name,
    stand: player.stand ? JSON.parse(JSON.stringify(player.stand)) : { key: "mandom" },
    synergies: player.synergies ? JSON.parse(JSON.stringify(player.synergies)) : createDefaultSynergies()
  };
}

function mandomTimelineRestoreSnapshotV1(snapshot, player, cooldownMs, now = mandomTimelineNowV1()) {
  const identity = mandomTimelineClonePlayerIdentityV1(player);
  const restored = JSON.parse(snapshot.raw);

  if (!restored.players || typeof restored.players !== "object") {
    restored.players = {};
  }

  if (!restored.players[player.id]) {
    restored.players[player.id] = JSON.parse(JSON.stringify(player));
  }

  restored.players[player.id].id = identity.id;
  restored.players[player.id].name = identity.name;
  restored.players[player.id].stand = identity.stand && identity.stand.key ? identity.stand : { key: "mandom" };
  restored.players[player.id].synergies = identity.synergies || createDefaultSynergies();
  restored.players[player.id].standCooldownUntil = now + cooldownMs;
  restored.players[player.id].activeStandBuff = null;

  normalizeAllPlayers(restored);

  if (typeof rebuildStateAggregates === "function") {
    rebuildStateAggregates(restored);
  }

  mandomTimelineRestoringV1 = true;

  try {
    saveState(restored);
  } finally {
    mandomTimelineRestoringV1 = false;
  }

  mandomTimelineLastActivationAtV1 = now;
  mandomTimelineSnapshotsV1 = [
    {
      at: now,
...
function mandomTimelineFallbackFishingOnlyV1(state, minutes) {
...
// ADMIN_MANDOM_TIMELINE_STATUS_V1_START

function adminRootMandomTimelineStatusV1() {
  const snapshots = Array.isArray(mandomTimelineSnapshotsV1) ? mandomTimelineSnapshotsV1 : [];
  const now = Date.now();

  const sorted = snapshots
    .filter((snapshot) => snapshot && Number(snapshot.at || 0) > 0)
    .sort((a, b) => Number(a.at || 0) - Number(b.at || 0));

  const oldest = sorted[0] || null;
  const newest = sorted[sorted.length - 1] || null;

  const oldestAge = oldest ? formatDurationCompact(Math.max(0, now - Number(oldest.at || now))) : "nenhum";
  const newestAge = newest ? formatDurationCompact(Math.max(0, now - Number(newest.at || now))) : "nenhum";
  const lastActivationAt = Number(mandomTimelineLastActivationAtV1 || 0);
  const lastActivationText = lastActivationAt > 0
    ? `${formatDurationCompact(Math.max(0, now - lastActivationAt))} atrás`
    : "nunca";

  const windowText = typeof MANDOM_TIMELINE_WINDOW_MS_V1 === "number"
    ? formatDurationCompact(MANDOM_TIMELINE_WINDOW_MS_V1)
    : "4min";

  const maxSnapshots = typeof MANDOM_TIMELINE_MAX_SNAPSHOTS_V1 === "number"
    ? MANDOM_TIMELINE_MAX_SNAPSHOTS_V1
    : "desconhecido";

  return [
    `⏪ *Mandom Timeline*`,
    ``,
    `Snapshots guardados: *${sorted.length}*`,
    `Limite máximo: *${maxSnapshots}*`,
    `Janela de rewind: *${windowText}*`,
    ``,
    `Snapshot mais antigo:`,
    `> ${oldest ? `${oldestAge} atrás` : "Nenhum"}`,
    ``,
    `Snapshot mais recente:`,
    `> ${newest ? `${newestAge} atrás` : "Nenhum"}`,
    ``,
    `Última ativação do Mandom:`,
    `> ${lastActivationText}`,
    ``,
    `Status:`,
    `> ${sorted.length ? "Timeline pronta para rebobinar ações salvas." : "Ainda sem snapshot em memória."}`,
    ``,
    `Observação:`,
    `> Esses snapshots ficam só na memória.`,
    `> Se reiniciar o bot, a timeline começa vazia.`
  ].join(adminRootNlV3());
}

// ADMIN_MANDOM_TIMELINE_STATUS_V1_END



// MANDOM_REWIND_SUMMARY_V1_START

function mandomTimelineCloneForSummaryV1(value) {
  return JSON.parse(JSON.stringify(value || {}));
}

function mandomTimelineGetPlayersV1(state) {
  return state && state.players && typeof state.players === "object" ? state.players : {};
}

function mandomTimelineGetSpecialItemV1(player, key) {
  const items = player && player.specialItems && typeof player.specialItems === "object" ? player.specialItems : {};
  return Number(items[key] || 0);
}

function mandomTimelineGetHolyPartsV1(player) {
  if (!player || !player.holyCorpse || !player.holyCorpse.parts || typeof player.holyCorpse.parts !== "object") {
    return {};
  }

  return player.holyCorpse.parts;
}

function mandomTimelineStandKeyV1(player) {
  return String(player?.stand?.key || "");
}

function mandomTimelineHistoryCountsV1(player) {
...
function mandomTimelineCountPartDiffsV1(beforePlayer, afterPlayer) {
  const beforeParts = mandomTimelineGetHolyPartsV1(beforePlayer);
  const afterParts = mandomTimelineGetHolyPartsV1(afterPlayer);
  const keys = new Set([...Object.keys(beforeParts), ...Object.keys(afterParts)]);
  let changed = 0;
...
function mandomTimelineEventStatusV1(state) {
...
function mandomTimelineBuildRewindSummaryV1(beforeState, afterState, mandomUserId) {
  const beforePlayers = mandomTimelineGetPlayersV1(beforeState);
  const afterPlayers = mandomTimelineGetPlayersV1(afterState);
  ...
  let baitsChanged = 0;
  let cooldownsChanged = 0;
  let standsChanged = 0;
  let activeBuffsChanged = 0;
  let specialItemsChanged = 0;
  let holyPartsChanged = 0;
  let totalWeightDelta = 0;

  for (const id of ids) {
    const beforePlayer = beforePlayers[id] || {};
    const afterPlayer = afterPlayers[id] || {};
    let changedThisPlayer = false;

    const beforeCounts = mandomTimelineHistoryCountsV1(beforePlayer);
    const afterCounts = mandomTimelineHistoryCountsV1(afterPlayer);

    const historyDelta = beforeCounts.total - afterCounts.total;

    if (historyDelta > 0) {
      capturesRemoved += historyDelta;
      changedThisPlayer = true;
    }

    const fishDelta = beforeCounts.fish - afterCounts.fish;
    const trashDelta = beforeCounts.trash - afterCounts.trash;
    const legendaryDelta = beforeCounts.legendary - afterCounts.legendary;
    const specialDelta = beforeCounts.special - afterCounts.special;

    if (fishDelta > 0) fishRemoved += fishDelta;
    if (trashDelta > 0) trashRemoved += trashDelta;
    if (legendaryDelta > 0) legendaryRemoved += legendaryDelta;
    if (specialDelta > 0) specialRemoved += specialDelta;

    const beforeWeight = Number(beforePlayer.totalWeight || 0);
    const afterWeight = Number(afterPlayer.totalWeight || 0);
    const weightDelta = beforeWeight - afterWeight;

    if (Math.abs(weightDelta) >= 0.01) {
      totalWeightDelta += weightDelta;
      changedThisPlayer = true;
    }
    ...
    if (String(beforePlayer.standCooldownUntil || "0") !== String(afterPlayer.standCooldownUntil || "0")) {
      if (id !== mandomUserId) {
        cooldownsChanged += 1;
      }
      changedThisPlayer = true;
    ...
    if (mandomTimelineStandKeyV1(beforePlayer) !== mandomTimelineStandKeyV1(afterPlayer)) {
      if (id !== mandomUserId) {
        standsChanged += 1;
      }
      changedThisPlayer = true;
    ...
    if (JSON.stringify(beforePlayer.activeStandBuff || null) !== JSON.stringify(afterPlayer.activeStandBuff || null)) {
      activeBuffsChanged += 1;
      changedThisPlayer = true;
    }

    const specialItemKeys = ["rokakaka", "blankStandDiscs", "standArrows"];
    const specialItemChangedHere = specialItemKeys.some((key) => {
      return mandomTimelineGetSpecialItemV1(beforePlayer, key) !== mandomTimelineGetSpecialItemV1(afterPlayer, key);
    });

    const beforeDiscs = JSON.stringify(beforePlayer?.specialItems?.standDiscs || []);
    const afterDiscs = JSON.stringify(afterPlayer?.specialItems?.standDiscs || []);

    if (specialItemChangedHere || beforeDiscs !== afterDiscs) {
      specialItemsChanged += 1;
      changedThisPlayer = true;
    }

    const partDiffs = mandomTimelineCountPartDiffsV1(beforePlayer, afterPlayer);

    if (partDiffs > 0) {
      holyPartsChanged += partDiffs;
      changedThisPlayer = true;
    }

    if (changedThisPlayer) {
      playersChanged += 1;
    }
  }

  const beforeEvent = mandomTimelineEventStatusV1(beforeState);
  const afterEvent = mandomTimelineEventStatusV1(afterState);
  const eventChanged = beforeEvent !== afterEvent;
  ...
  if (specialRemoved > 0) {
    lines.push(`> Itens pescados revertidos: *${specialRemoved}*`);
  }

  if (Math.abs(totalWeightDelta) >= 0.01) {
    lines.push(`> Peso total revertido: *${formatWeight(Math.max(0, totalWeightDelta))}*`);
  }

  lines.push(
    `> Iscas alteradas: *${baitsChanged}*`,
    `> Cooldowns de Stand revertidos: *${cooldownsChanged}*`,
    `> Stands revertidos: *${standsChanged}*`,
    `> Buffs ativos revertidos: *${activeBuffsChanged}*`,
    `> Inventários especiais alterados: *${specialItemsChanged}*`,
    `> Partes do Cadáver alteradas: *${holyPartsChanged}*`
  );

  if (eventChanged) {
    lines.push(`> Evento do Cadáver Santo: *revertido*`);
  } else {
    lines.push(`> Evento do Cadáver Santo: *sem mudança*`);
  }
...
// MANDOM_REWIND_SUMMARY_V1_END


// MANDOM_TIMELINE_REWIND_V1_END
...
function saveState(state) {
  ...
  ensureDataFile();
  captureMandomReliableSnapshotBeforeSaveV4();
  captureMandomFullSnapshotBeforeSaveV3();
  mandomTimelineCaptureBeforeSaveV2();
  mandomTimelineCaptureBeforeSaveV1();
...
// MANDOM_FULL_GROUP_REWIND_V2_START

const MANDOM_TIMELINE_FILE_V2 = path.join(DATA_DIR, "mandom_timeline.json");
const MANDOM_TIMELINE_WINDOW_MS_V2 = 4 * 60 * 1000;
const MANDOM_TIMELINE_KEEP_EXTRA_MS_V2 = 45 * 1000;
const MANDOM_TIMELINE_MAX_SNAPSHOTS_V2 = 240;

let mandomTimelineRestoringV2 = false;

function mandomFullNlV2() {
  return String.fromCharCode(10);
}

function mandomFullCloneV2(value) {
  return JSON.parse(JSON.stringify(value || null));
}

function mandomFullReadTimelineV2() {
  try {
    if (!fs.existsSync(MANDOM_TIMELINE_FILE_V2)) {
      return [];
    }

    const raw = fs.readFileSync(MANDOM_TIMELINE_FILE_V2, "utf8");
    const parsed = JSON.parse(raw);

    return Array.isArray(parsed) ? parsed : [];
  } catch (error) {
    log("Mandom V2: erro ao ler timeline:", error.message);
    return [];
  }
}

function mandomFullWriteTimelineV2(snapshots) {
  try {
    ...
    fs.writeFileSync(
      MANDOM_TIMELINE_FILE_V2,
      JSON.stringify(snapshots, null, 2),
      "utf8"
    );
  } catch (error) {
    log("Mandom V2: erro ao salvar timeline:", error.message);
  }
}

function mandomFullTrimTimelineV2(snapshots, now = Date.now()) {
  const minAt = now - MANDOM_TIMELINE_WINDOW_MS_V2 - MANDOM_TIMELINE_KEEP_EXTRA_MS_V2;

  return snapshots
    .filter((snapshot) => snapshot && Number(snapshot.at || 0) >= minAt && snapshot.raw)
    .sort((a, b) => Number(a.at || 0) - Number(b.at || 0))
    .slice(-MANDOM_TIMELINE_MAX_SNAPSHOTS_V2);
}

function mandomTimelineCaptureBeforeSaveV2() {
  if (mandomTimelineRestoringV2) {
    return;
  }
  ...
  try {
    ...
    const now = Date.now();
    let snapshots = mandomFullReadTimelineV2();
    snapshots = mandomFullTrimTimelineV2(snapshots, now);

    const last = snapshots[snapshots.length - 1];

    if (last && last.raw === compactRaw) {
      last.at = now;
      mandomFullWriteTimelineV2(snapshots);
      return;
    }
    ...
    log("Mandom V2: não consegui capturar snapshot:", error.message);
  }
}

function mandomFullPickSnapshotV2(now = Date.now()) {
  let snapshots = mandomFullReadTimelineV2();
  snapshots = mandomFullTrimTimelineV2(snapshots, now);
  mandomFullWriteTimelineV2(snapshots);
  ...
  const targetAt = now - MANDOM_TIMELINE_WINDOW_MS_V2;

  const beforeTarget = snapshots
...
function mandomFullCountHistoryV2(player) {
  const history = Array.isArray(player?.history) ? player.history : [];
...
function mandomFullEventStatusV2(state) {
  const event = state?.holyCorpseEvent;
...
function mandomFullBuildSummaryV2(beforeState, restoredState, actorId) {
  ...
  for (const id of ids) {
    ...
    const beforeCount = mandomFullCountHistoryV2(beforePlayer);
    const afterCount = mandomFullCountHistoryV2(afterPlayer);

    const captureDelta = beforeCount.total - afterCount.total;
    const fishDelta = beforeCount.fish - afterCount.fish;
    const trashDelta = beforeCount.trash - afterCount.trash;
    const legendaryDelta = beforeCount.legendary - afterCount.legendary;
  ...
  const eventChanged = mandomFullEventStatusV2(beforeState) !== mandomFullEventStatusV2(restoredState);
...
async function activateMandomOldDisabledV3_1(state, player) {
  const now = Date.now();
  const cooldownMs = getStandCooldownMs(player) || (4 * 60 * 1000);
  const snapshot = mandomFullPickSnapshotV2(now);
  ...
  if (!snapshot) {
    ...
    return [
      ...
      ``,
      `Mas ainda não existe uma timeline suficiente salva.`,
      `> A timeline começa a ser registrada automaticamente a cada alteração do jogo.`,
      `> Tente novamente depois de alguns minutos.`
    ].join(mandomFullNlV2());
  }

  const beforeState = mandomFullCloneV2(state);
  ...
  const currentActor = mandomFullCloneV2(player);
  ...
  mandomTimelineRestoringV2 = true;

  try {
    saveState(restoredState);
  } finally {
    mandomTimelineRestoringV2 = false;
  }

  mandomFullWriteTimelineV2([
    {
      at: now,
  ...
  const summary = mandomFullBuildSummaryV2(beforeState, restoredState, player.id);
...
// MANDOM_FULL_GROUP_REWIND_V2_END



// MANDOM_FORCE_FULL_REWIND_V3_START

const MANDOM_FULL_REWIND_FILE_V3 = path.join(DATA_DIR, "mandom_timeline.json");
const MANDOM_FULL_REWIND_WINDOW_MS_V3 = 4 * 60 * 1000;
const MANDOM_FULL_REWIND_KEEP_EXTRA_MS_V3 = 60 * 1000;
const MANDOM_FULL_REWIND_MAX_SNAPSHOTS_V3 = 240;

let mandomFullRewindRestoringV3 = false;

function mandomFullRewindNlV3() {
  return String.fromCharCode(10);
}

function mandomFullRewindCloneV3(value) {
  return JSON.parse(JSON.stringify(value || null));
}

function readMandomFullTimelineV3() {
  try {
    if (!fs.existsSync(MANDOM_FULL_REWIND_FILE_V3)) {
      return [];
    }

    const raw = fs.readFileSync(MANDOM_FULL_REWIND_FILE_V3, "utf8");
    const parsed = JSON.parse(raw);

    return Array.isArray(parsed) ? parsed : [];
  } catch (error) {
    log("Mandom V3: erro ao ler timeline:", error.message);
    return [];
  }
}

function writeMandomFullTimelineV3(snapshots) {
  try {
    if (!fs.existsSync(DATA_DIR)) {
      fs.mkdirSync(DATA_DIR, { recursive: true });
    }
    ...
    fs.writeFileSync(
      MANDOM_FULL_REWIND_FILE_V3,
      JSON.stringify(snapshots, null, 2),
      "utf8"
    );
  } catch (error) {
    log("Mandom V3: erro ao salvar timeline:", error.message);
  }
}

function trimMandomFullTimelineV3(snapshots, now = Date.now()) {
  const minAt = now - MANDOM_FULL_REWIND_WINDOW_MS_V3 - MANDOM_FULL_REWIND_KEEP_EXTRA_MS_V3;

  return snapshots
    .filter((snapshot) => snapshot && Number(snapshot.at || 0) >= minAt && snapshot.raw)
    .sort((a, b) => Number(a.at || 0) - Number(b.at || 0))
    .slice(-MANDOM_FULL_REWIND_MAX_SNAPSHOTS_V3);
}

function captureMandomFullSnapshotBeforeSaveV3() {
  if (mandomFullRewindRestoringV3) {
    return;
  }
  ...
  try {
    ...
    const now = Date.now();
    let snapshots = readMandomFullTimelineV3();
    snapshots = trimMandomFullTimelineV3(snapshots, now);

    const last = snapshots[snapshots.length - 1];

    if (last && last.raw === compactRaw) {
      last.at = now;
      writeMandomFullTimelineV3(snapshots);
      return;
    }
    ...
    writeMandomFullTimelineV3(trimMandomFullTimelineV3(snapshots, now));
  } catch (error) {
    log("Mandom V3: não consegui capturar snapshot:", error.message);
  }
}

function pickMandomFullSnapshotV3(now = Date.now()) {
  let snapshots = readMandomFullTimelineV3();
  snapshots = trimMandomFullTimelineV3(snapshots, now);
  writeMandomFullTimelineV3(snapshots);
  ...
  const targetAt = now - MANDOM_FULL_REWIND_WINDOW_MS_V3;
...
function countMandomHistoryV3(player) {
  const history = Array.isArray(player?.history) ? player.history : [];
...
function getMandomEventStatusV3(state) {
  const event = state?.holyCorpseEvent;
...
function buildMandomFullSummaryV3(beforeState, restoredState, actorId) {
  const beforePlayers = beforeState?.players || {};
  const afterPlayers = restoredState?.players || {};
  const ids = new Set([...Object.keys(beforePlayers), ...Object.keys(afterPlayers)]);
  ...
  for (const id of ids) {
    ...
    const beforeCount = countMandomHistoryV3(beforePlayer);
    const afterCount = countMandomHistoryV3(afterPlayer);
  ...
  const eventChanged = getMandomEventStatusV3(beforeState) !== getMandomEventStatusV3(restoredState);
...
async function activateMandom(state, player) {
  const now = Date.now();
  const cooldownMs = getStandCooldownMs(player) || (4 * 60 * 1000);
  const snapshot = pickMandomFullSnapshotV3(now);

  if (!snapshot) {
    return [
      `⏪ *Mandom tentou rebobinar o tempo...*`,
      ``,
      `Mas ainda não existe timeline suficiente salva.`,
      `> Nenhum cooldown foi aplicado.`,
      `> Tente novamente depois de alguns minutos com o bot rodando.`
    ].join(mandomFullRewindNlV3());
  }

  const beforeState = mandomFullRewindCloneV3(state);
  const restoredState = JSON.parse(snapshot.raw);
  const snapshotAgeMs = Math.max(0, now - Number(snapshot.at || now));
  ...
  const currentActor = mandomFullRewindCloneV3(player);
  const restoredActor = restoredState.players[player.id] || currentActor || {};
  ...
  mandomFullRewindRestoringV3 = true;

  try {
    saveState(restoredState);
  } finally {
    mandomFullRewindRestoringV3 = false;
  }

  writeMandomFullTimelineV3([
    {
      at: now,
      raw: JSON.stringify(restoredState)
    }
  ...
  const summary = buildMandomFullSummaryV3(beforeState, restoredState, player.id);
...
// MANDOM_FORCE_FULL_REWIND_V3_END



// MANDOM_RELIABLE_FULL_REWIND_V4_START

const MANDOM_RELIABLE_TIMELINE_FILE_V4 = path.join(DATA_DIR, "mandom_timeline.json");
const MANDOM_RELIABLE_WINDOW_MS_V4 = 4 * 60 * 1000;
const MANDOM_RELIABLE_KEEP_EXTRA_MS_V4 = 60 * 1000;
const MANDOM_RELIABLE_MAX_SNAPSHOTS_V4 = 240;

let mandomReliableRestoringV4 = false;

function mandomReliableNlV4() {
  return String.fromCharCode(10);
}

function mandomReliableCloneV4(value) {
  return JSON.parse(JSON.stringify(value || null));
}

function readMandomReliableTimelineV4() {
  try {
    if (!fs.existsSync(MANDOM_RELIABLE_TIMELINE_FILE_V4)) {
      return [];
    }

    const raw = fs.readFileSync(MANDOM_RELIABLE_TIMELINE_FILE_V4, "utf8");
    const parsed = JSON.parse(raw);

    return Array.isArray(parsed) ? parsed : [];
  } catch (error) {
    log("Mandom V4: erro ao ler timeline:", error.message);
    return [];
  }
}

function writeMandomReliableTimelineV4(snapshots) {
  try {
    if (!fs.existsSync(DATA_DIR)) {
      fs.mkdirSync(DATA_DIR, { recursive: true });
    }
    ...
    fs.writeFileSync(
      MANDOM_RELIABLE_TIMELINE_FILE_V4,
      JSON.stringify(snapshots, null, 2),
      "utf8"
    );
  } catch (error) {
    log("Mandom V4: erro ao salvar timeline:", error.message);
  }
}

function trimMandomReliableTimelineV4(snapshots, now = Date.now()) {
  const minAt = now - MANDOM_RELIABLE_WINDOW_MS_V4 - MANDOM_RELIABLE_KEEP_EXTRA_MS_V4;

  return snapshots
    .filter((snapshot) => snapshot && Number(snapshot.at || 0) >= minAt && snapshot.raw)
    .sort((a, b) => Number(a.at || 0) - Number(b.at || 0))
    .slice(-MANDOM_RELIABLE_MAX_SNAPSHOTS_V4);
}

function captureMandomReliableSnapshotBeforeSaveV4() {
  if (mandomReliableRestoringV4) {
    return;
  }
  ...
  try {
    ...
    const now = Date.now();
    let snapshots = readMandomReliableTimelineV4();
    snapshots = trimMandomReliableTimelineV4(snapshots, now);

    const last = snapshots[snapshots.length - 1];

    if (last && last.raw === compactRaw) {
      last.at = now;
      writeMandomReliableTimelineV4(snapshots);
      return;
    }
    ...
    writeMandomReliableTimelineV4(trimMandomReliableTimelineV4(snapshots, now));
  } catch (error) {
    log("Mandom V4: não consegui capturar snapshot:", error.message);
  }
}

function pickMandomReliableSnapshotV4(now = Date.now()) {
  let snapshots = readMandomReliableTimelineV4();
  snapshots = trimMandomReliableTimelineV4(snapshots, now);
  writeMandomReliableTimelineV4(snapshots);
  ...
  const targetAt = now - MANDOM_RELIABLE_WINDOW_MS_V4;

  const olderOrEqual = snapshots
    .filter((snapshot) => Number(snapshot.at || 0) <= targetAt)
    .sort((a, b) => Number(a.at || 0) - Number(b.at || 0));
...
function countMandomReliableHistoryV4(player) {
  const history = Array.isArray(player?.history) ? player.history : [];
...
function getMandomReliableEventStatusV4(state) {
  const event = state?.holyCorpseEvent;
...
function buildMandomReliableSummaryV4(beforeState, restoredState, actorId) {
  const beforePlayers = beforeState?.players || {};
  const afterPlayers = restoredState?.players || {};
  const ids = new Set([...Object.keys(beforePlayers), ...Object.keys(afterPlayers)]);
  ...
  for (const id of ids) {
    ...
    const beforeCount = countMandomReliableHistoryV4(beforePlayer);
    const afterCount = countMandomReliableHistoryV4(afterPlayer);
  ...
  const eventChanged = getMandomReliableEventStatusV4(beforeState) !== getMandomReliableEventStatusV4(restoredState);
...
async function forceActivateMandomReliableV4(state, player) {
  const now = Date.now();
  const cooldownMs = getStandCooldownMs(player) || (4 * 60 * 1000);
  const snapshot = pickMandomReliableSnapshotV4(now);
  ...
  if (!snapshot) {
    return {
      ...
      ].join(mandomReliableNlV4())
    };
  }

  const beforeState = mandomReliableCloneV4(state);
  const restoredState = JSON.parse(snapshot.raw);
  const snapshotAgeMs = Math.max(0, now - Number(snapshot.at || now));
  ...
  const currentActor = mandomReliableCloneV4(player);
  const restoredActor = restoredState.players[player.id] || currentActor || {};
  ...
  mandomReliableRestoringV4 = true;

  try {
    saveState(restoredState);
  } finally {
    mandomReliableRestoringV4 = false;
  }

  writeMandomReliableTimelineV4([
    {
      at: now,
      raw: JSON.stringify(restoredState)
    }
  ...
  const summary = buildMandomReliableSummaryV4(beforeState, restoredState, player.id);
...
// MANDOM_RELIABLE_FULL_REWIND_V4_END
...
async function handleStandActivate(message, state, player, arg) {
  ...
  if (stand.key === "mandom") {
    ...
    if (typeof forceActivateMandomReliableV4 === "function") {
      result = await forceActivateMandomReliableV4(state, player);
    } else if (typeof activateMandom === "function") {
      const fallbackText = await activateMandom(state, player);
      result = {
        appliedCooldown: true,
        text: String(fallbackText || "⏪ Mandom foi ativado.")
      };
    } else {
      result = {
        appliedCooldown: false,
        text: "⏪ Mandom está instalado, mas a função de ativação não foi encontrada."
      };
    }

    const text = typeof result === "string"
      ? result
      : String(result?.text || "⏪ Mandom foi ativado.");
