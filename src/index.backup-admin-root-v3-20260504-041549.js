...
// PRIVATE_ADMIN_ROOT_V2_START

const ADMIN_ROOT_PHONE_V2 = "24998805233";
const ADMIN_ROOT_COMMANDS_V2 = new Set(["!admin", "!adm", "!root"]);

function adminRootNlV2() {
  return String.fromCharCode(10);
}

function adminRootDigitsV2(value) {
  return String(value || "").replace(/\D/g, "");
}

function adminRootNormV2(value) {
  return String(value || "")
    .trim()
    .toLowerCase()
    .normalize("NFD")
...
function adminRootIsOwnerV2(message) {
  const ownerPhone = adminRootDigitsV2(ADMIN_ROOT_PHONE_V2);
  ...
  return values.some((value) => {
    ...
    const digits = adminRootDigitsV2(raw);

    if (!raw && !digits) {
      return false;
    }

    if (allowedExactIds.has(raw)) {
      return true;
    }

    return Array.from(allowedDigits).some((allowed) => {
      return Boolean(allowed) && Boolean(digits) && (
        digits === allowed ||
        digits.endsWith(allowed)
      );
    });
  });
}

function adminRootShortIdV2(id) {
  const digits = adminRootDigitsV2(id);
  return digits ? `...${digits.slice(-6)}` : String(id || "");
}

async function adminRootReplyV2(message, text) {
  await message.reply(String(text || ""));
}

function adminRootMenuV2() {
  return [
    `🗝️ *ROOT PRIVADO — Bot da Pescaria*`,
    ``,
    `Acesso aceito apenas no privado do dono.`,
    `Nenhuma confirmação é enviada no grupo.`,
    ``,
    `📋 *Consulta*`,
    `• !admin status`,
    `• !admin jogadores`,
...
function adminRootResolvePlayerV2(state, rawQuery) {
  const query = String(rawQuery || "").trim();
  const queryDigits = adminRootDigitsV2(query);
  const queryText = adminRootNormV2(query);

  if (!query) return { error: "Informe um alvo." };

  const players = Object.values(state.players || {});

  const exact = players.find((player) => {
    const id = String(player.id || "");
    const idDigits = adminRootDigitsV2(id);
    const name = adminRootNormV2(player.name);

    return id === query ||
      idDigits === queryDigits ||
      (queryDigits.length >= 6 && idDigits.endsWith(queryDigits)) ||
      name === queryText;
  });
  ...
  const partial = players.filter((player) => {
    const idDigits = adminRootDigitsV2(player.id);
    const name = adminRootNormV2(player.name);

    return (queryDigits.length >= 4 && idDigits.includes(queryDigits)) ||
      (queryText.length >= 2 && name.includes(queryText));
  });
  ...
  if (partial.length > 1) {
    return {
      error: [
        `Alvo ambíguo: ${query}`,
        ``,
        ...partial.slice(0, 12).map((player) => `• ${player.name} — ${adminRootShortIdV2(player.id)}`)
      ].join(adminRootNlV2())
    };
  }
...
function adminRootCommitV2(state) {
  normalizeAllPlayers(state);
...
function adminRootParseNumberV2(value, fallback = null) {
  const parsed = Number(String(value || "").replace(",", "."));
  return Number.isFinite(parsed) ? parsed : fallback;
}

function adminRootPlayerStandV2(player) {
  const key = player?.stand?.key;
  const stand = key ? STAND_DEFS[key] : null;
  return stand ? `${stand.emoji} ${stand.name} (${key})` : "Nenhum";
}

function adminRootStatusV2(state) {
  return [
    `🗝️ *ROOT ativo*`,
    ``,
    `Grupo permitido: ${ALLOWED_GROUP_ID || "não configurado"}`,
    ...
    ``,
    `Dono: ${ADMIN_ROOT_PHONE_V2}`
  ].join(adminRootNlV2());
}

function adminRootListPlayersV2(state) {
  const rows = Object.values(state.players || {})
    .sort((a, b) => Number(b.totalWeight || 0) - Number(a.totalWeight || 0))
    .map((player, index) => {
      return `${index + 1}. ${player.name} — ${adminRootShortIdV2(player.id)} — ${formatWeight(player.totalWeight || 0)} — ${Number(player.baits || 0)}/${getMaxBaits(player)} iscas — ${adminRootPlayerStandV2(player)}`;
    });

  return [
    `👥 *Jogadores*`,
    ``,
    rows.length ? rows.join(adminRootNlV2()) : `_Nenhum jogador cadastrado._`
  ].join(adminRootNlV2());
}

function adminRootViewPlayerV2(state, player) {
  ...
  const parts = player.holyCorpse?.parts
    ? Object.entries(player.holyCorpse.parts).map(([key, amount]) => `${key}: ${amount}`).join(" | ")
    : "nenhuma";
  ...
  return [
    ...
    `Stand: ${adminRootPlayerStandV2(player)}`,
...
function adminRootChangeBaitsV2(state, args, additive) {
  const [targetRaw, amountRaw] = args;
  const amount = adminRootParseNumberV2(amountRaw, null);

  if (!targetRaw || amount === null) {
    return `Uso: !admin ${additive ? "add-iscas" : "iscas"} <alvo> <qtd>`;
  }

  const resolved = adminRootResolvePlayerV2(state, targetRaw);
  if (!resolved.player) return resolved.error;

  const player = resolved.player;
  const before = Number(player.baits || 0);

  player.baits = Math.max(0, Math.min(999, Math.trunc(additive ? before + amount : amount)));
  player.lastBaitAt = Date.now();

  adminRootCommitV2(state);

  return `✅ Iscas de ${player.name}: ${before} → ${player.baits}`;
}

function adminRootSetStandV2(state, args) {
  const [targetRaw, standRaw] = args;

  if (!targetRaw || !standRaw) return `Uso: !admin stand <alvo> <stand_key|none>`;

  const resolved = adminRootResolvePlayerV2(state, targetRaw);
  if (!resolved.player) return resolved.error;

  const player = resolved.player;
  const key = adminRootNormV2(standRaw).replace(/[^a-z0-9_:-]/g, "_").replace(/:+/g, "_");
  ...
  if (["none", "nenhum", "remover", "null", "0"].includes(key)) {
    ...
    player.activeStandBuff = null;
    adminRootCommitV2(state);
    return `✅ Stand removido de ${player.name}.`;
  }

  if (!STAND_DEFS[key]) return `⛔ Stand inválido: ${standRaw}`;
  ...
  return `✅ ${player.name} recebeu ${STAND_DEFS[key].emoji} *${STAND_DEFS[key].name}*.`;
}

function adminRootChangeItemV2(state, args) {
  const [targetRaw, itemRaw, amountRaw] = args;
  const amount = Math.trunc(adminRootParseNumberV2(amountRaw, 0));

  if (!targetRaw || !itemRaw || !amount) return `Uso: !admin item <alvo> rokakaka|disco-vazio <qtd>`;
  ...
  const item = adminRootNormV2(itemRaw).replace(/_/g, "-");
  ...
  if (item === "disco-vazio" || item === "disco") {
...
function adminRootAddStandDiscV2(state, args) {
  const [targetRaw, standRaw, amountRaw] = args;
  const amount = Math.max(1, Math.min(20, Math.trunc(adminRootParseNumberV2(amountRaw, 1))));

  if (!targetRaw || !standRaw) return `Uso: !admin disco-stand <alvo> <stand_key> [qtd]`;
  ...
  return `✅ ${amount}x disco de ${STAND_DEFS[key].name} adicionado para ${player.name}.`;
}

function adminRootChangeCadaverV2(state, args) {
  const [targetRaw, partRaw, amountRaw] = args;
  const amount = Math.trunc(adminRootParseNumberV2(amountRaw, 1));

  if (!targetRaw || !partRaw) return `Uso: !admin cadaver <alvo> <parte> [qtd]`;

  if (typeof normalizeHolyCorpsePartKey !== "function" || typeof ensureHolyCorpseInventory !== "function") {
    return `⛔ Sistema do Cadáver Santo não encontrado.`;
  }
  ...
  const key = normalizeHolyCorpsePartKey(partRaw);
  if (!HOLY_CORPSE_PARTS[key]) return `⛔ Parte inválida: ${partRaw}`;

  const before = Number(player.holyCorpse.parts[key] || 0);
  player.holyCorpse.parts[key] = Math.max(0, before + amount);

  adminRootCommitV2(state);

  return `✅ ${HOLY_CORPSE_PARTS[key].emoji} ${HOLY_CORPSE_PARTS[key].name} de ${player.name}: ${before} → ${player.holyCorpse.parts[key]}`;
}

function adminRootAddFishV2(state, args) {
  const [targetRaw, kgRaw, ...nameParts] = args;
  const kg = adminRootParseNumberV2(kgRaw, null);

  if (!targetRaw || kg === null || kg <= 0) return `Uso: !admin peixe <alvo> <kg> [nome]`;

  const resolved = adminRootResolvePlayerV2(state, targetRaw);
  if (!resolved.player) return resolved.error;

  const player = resolved.player;
  const name = nameParts.join(" ").trim() || "Peixe Administrativo";
  ...
  return `✅ ${player.name} recebeu 🐟 ${name} — ${formatWeight(kg)}.`;
}

function adminRootResetCooldownsV2(state, args) {
  const [targetRaw] = args;

  if (!targetRaw) return `Uso: !admin reset-cooldowns <alvo>`;
  ...
  return `✅ Cooldowns resetados para ${player.name}.`;
}

function adminRootResetPintoV2(state, args) {
  const [targetRaw] = args;

  if (!targetRaw) return `Uso: !admin pinto-reset <alvo>`;
  ...
  return `✅ Pinto resetado para ${player.name}.`;
}

function adminRootBackupV2() {
  ensureDataFile();
  const stamp = new Date().toISOString().replace(/[:.]/g, "-");
  const backupPath = path.join(DATA_DIR, `pesca.admin-root-${stamp}.json`);
  fs.copyFileSync(DATA_FILE, backupPath);
  return `✅ Backup criado: ${backupPath}`;
}

async function adminRootBanV2(state, args) {
  const [targetRaw] = args;

  if (!targetRaw) return `Uso: !admin banir <nome|telefone|id>`;
  if (!ALLOWED_GROUP_ID) return `⛔ Grupo permitido não configurado.`;

  const resolved = adminRootResolvePlayerV2(state, targetRaw);
  let targetId = resolved.player?.id || "";

  if (!targetId) {
    const digits = adminRootDigitsV2(targetRaw);
    if (!digits) return resolved.error || `Não achei alvo: ${targetRaw}`;
    targetId = digits.startsWith("55") ? `${digits}@c.us` : `55${digits}@c.us`;
  }

  const groupChat = await client.getChatById(ALLOWED_GROUP_ID);

  if (!groupChat || !groupChat.isGroup) return `⛔ Não consegui abrir o grupo.`;
  if (typeof groupChat.removeParticipants !== "function") return `⛔ removeParticipants indisponível nessa versão.`;
  ...
  return [
    `✅ Remoção solicitada.`,
    `Alvo: ${targetId}`,
    `O bot não mandou mensagem no grupo.`
  ].join(adminRootNlV2());
}

async function handleAdminRootCommandV2(message, arg) {
  const [subRaw, ...args] = String(arg || "").trim().split(/\s+/).filter(Boolean);
  const sub = adminRootNormV2(subRaw || "menu").replace(/_/g, "-");

  const state = loadState();
  normalizeAllPlayers(state);

  if (["", "menu", "help", "ajuda"].includes(sub)) {
    await adminRootReplyV2(message, adminRootMenuV2());
    return;
  }

  if (sub === "status") {
    await adminRootReplyV2(message, adminRootStatusV2(state));
    return;
  }

  if (["jogadores", "players", "listar"].includes(sub)) {
    await adminRootReplyV2(message, adminRootListPlayersV2(state));
    return;
  }

  if (["ver", "info"].includes(sub)) {
    const resolved = adminRootResolvePlayerV2(state, args[0]);
    await adminRootReplyV2(message, resolved.player ? adminRootViewPlayerV2(state, resolved.player) : resolved.error);
    return;
  }

  if (sub === "iscas") {
    await adminRootReplyV2(message, adminRootChangeBaitsV2(state, args, false));
    return;
  }

  if (["add-iscas", "somar-iscas"].includes(sub)) {
    await adminRootReplyV2(message, adminRootChangeBaitsV2(state, args, true));
    return;
  }

  if (["peixe", "add-peixe", "pontuacao", "pontuação"].includes(sub)) {
    await adminRootReplyV2(message, adminRootAddFishV2(state, args));
    return;
  }

  if (sub === "stand") {
    await adminRootReplyV2(message, adminRootSetStandV2(state, args));
    return;
  }

  if (["item", "itens"].includes(sub)) {
    await adminRootReplyV2(message, adminRootChangeItemV2(state, args));
    return;
  }

  if (["disco-stand", "stand-disco"].includes(sub)) {
    await adminRootReplyV2(message, adminRootAddStandDiscV2(state, args));
    return;
  }

  if (["cadaver", "cadáver", "parte"].includes(sub)) {
    await adminRootReplyV2(message, adminRootChangeCadaverV2(state, args));
    return;
  }

  if (["reset-cooldowns", "reset-cooldown", "cooldowns"].includes(sub)) {
    await adminRootReplyV2(message, adminRootResetCooldownsV2(state, args));
    return;
  }

  if (sub === "pinto-reset") {
    await adminRootReplyV2(message, adminRootResetPintoV2(state, args));
    return;
  }

  if (sub === "backup") {
    await adminRootReplyV2(message, adminRootBackupV2());
    return;
  }

  if (["banir", "ban", "remover"].includes(sub)) {
    await adminRootReplyV2(message, await adminRootBanV2(state, args));
    return;
  }

  await adminRootReplyV2(message, `⛔ Comando admin desconhecido: ${subRaw}${adminRootNlV2()}Use: !admin`);
}

async function adminRootPrivateGateV2(message, chat, rawBody) {
  if (!rawBody.startsWith(COMMAND_PREFIX)) return false;

  const parsed = parseCommand(rawBody);

  if (!ADMIN_ROOT_COMMANDS_V2.has(parsed.command)) return false;
  ...
  const owner = adminRootIsOwnerV2(message);

  console.log(`[admin-root] comando privado recebido: ${parsed.command} from=${from} to=${to} owner=${owner}`);
  ...
  try {
    await handleAdminRootCommandV2(message, parsed.arg);
  } catch (error) {
    console.error("[admin-root] erro:", error);
    try {
      await adminRootReplyV2(
        message,
        [
...
client.on("message", async (message) => {
  try {
    ...
    if (await adminRootPrivateGateV2(message, chat, rawBody)) return;
