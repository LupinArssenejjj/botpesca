...
async function adminRootPrivateGateV3(message, chat, rawBody) {
  ...
  if (!chat?.isGroup && adminRootIsOwnerV3(message)) {
    ...
    if (session && !ADMIN_ROOT_COMMANDS_V3.has(parseCommand(body).command)) {
      ...
      if (session.action?.type === "spawnCadaver") {
        await adminRootHandleCadaverSpawnSessionV1(message, body, session);
      } else {
...
// ADMIN_ROOT_CADAVER_SPAWN_V1_START

const ADMIN_ROOT_CADAVER_SPAWN_TIMERS_V1 = new Map();
let ADMIN_ROOT_CADAVER_SPAWN_SEQ_V1 = 1;

function adminRootCadaverSpawnPartAliasesV1() {
  return [
    { key: "rib_cage", aliases: ["rib_cage", "rib cage", "caixa toracica", "caixa torácica", "costela", "rib"] },
    { key: "left_arm", aliases: ["left_arm", "left arm", "braco esquerdo", "braço esquerdo", "braco", "braço", "arm"] },
    { key: "eye", aliases: ["olho santo", "olho", "eye"] },
    { key: "heart", aliases: ["coracao santo", "coração santo", "coracao", "coração", "heart"] },
    { key: "spine", aliases: ["espinha", "coluna", "spine"] },
    { key: "skull", aliases: ["cranio", "crânio", "caveira", "skull"] },
    { key: "legs", aliases: ["pernas", "perna", "legs"] }
  ];
}

function adminRootNormalizeCadaverPartKeyV1(value) {
  const raw = String(value || "").trim();

  if (!raw) return "";

  if (typeof normalizeHolyCorpsePartKey === "function") {
    return normalizeHolyCorpsePartKey(raw);
  }

  const clean = adminRootNormV3(raw).replace(/_/g, " ");

  for (const item of adminRootCadaverSpawnPartAliasesV1()) {
    if (item.aliases.map((alias) => adminRootNormV3(alias).replace(/_/g, " ")).includes(clean)) {
      return item.key;
    }
  }
...
function adminRootGetCadaverPartV1(partKey) {
  const key = adminRootNormalizeCadaverPartKeyV1(partKey);

  if (typeof HOLY_CORPSE_PARTS === "object" && HOLY_CORPSE_PARTS[key]) {
    return HOLY_CORPSE_PARTS[key];
  }

  if (typeof getHolyCorpsePartDefV1 === "function") {
    return getHolyCorpsePartDefV1(key);
  }
...
function adminRootExtractCadaverPartFromTextV1(text) {
  const clean = adminRootNormV3(text).replace(/_/g, " ");

  for (const item of adminRootCadaverSpawnPartAliasesV1()) {
    for (const alias of item.aliases) {
      const normalizedAlias = adminRootNormV3(alias).replace(/_/g, " ");

      if (clean === normalizedAlias || clean.includes(normalizedAlias)) {
        return item.key;
      }
    }
  }

  const direct = adminRootNormalizeCadaverPartKeyV1(clean);
  const part = adminRootGetCadaverPartV1(direct);

  return part ? part.key : "";
}

function adminRootParseCadaverDelayMsV1(text) {
  const clean = adminRootNormV3(text)
    .replace(/\bdaqui\b/g, " ")
    .replace(/\bdaq\b/g, " ")
    .replace(/\bem\b/g, " ")
    .replace(/\ba\b/g, " ")
  ...
  if (["agora", "imediato", "imediatamente", "ja", "já"].includes(clean)) {
    return 0;
  }
  ...
  const unit = adminRootNormV3(match[2] || "minuto");

  if (["segundo", "segundos", "seg", "sec", "secs", "s"].includes(unit)) {
    return Math.round(amount * 1000);
  }

  if (["hora", "horas", "h"].includes(unit)) {
...
function adminRootFormatDelayV1(delayMs) {
...
function adminRootCadaverPartListTextV1() {
  const parts = typeof HOLY_CORPSE_PARTS === "object"
    ? Object.values(HOLY_CORPSE_PARTS)
    : [
      { key: "eye", emoji: "👁️", name: "Olho Santo" },
      { key: "heart", emoji: "❤️", name: "Coração Santo" },
      { key: "left_arm", emoji: "💪", name: "Left Arm" },
      { key: "spine", emoji: "🦴", name: "Spine" },
      { key: "rib_cage", emoji: "🫁", name: "Rib Cage" },
      { key: "skull", emoji: "🧠", name: "Skull" },
      { key: "legs", emoji: "🦵", name: "Legs" }
    ];

  return parts
    .map((part, index) => `${index + 1}. ${part.emoji} ${part.name} — ${part.key}`)
    .join(adminRootNlV3());
}

function adminRootCadaverSpawnAskPartTextV1() {
  return [
    `✨ *Spawnar Cadáver Santo*`,
    ``,
    `Escolha qual parte vai aparecer no grupo.`,
    ``,
    adminRootCadaverPartListTextV1(),
    ``,
    `0. Voltar`,
    ``,
    `> Envie o nome, key ou número.`,
    `> Exemplo: *espinha*`,
    `> Exemplo: *olho*`,
    `> Use *cancelar* para sair.`
  ].join(adminRootNlV3());
...
function adminRootCadaverSpawnAskDelayTextV1(part) {
  return [
    `✨ *Spawnar Cadáver Santo*`,
    ``,
    `Parte escolhida: ${part.emoji} *${part.name}*`,
    ``,
    `Quando ela deve aparecer no grupo?`,
    ``,
    `Exemplos:`,
    `> agora`,
    `> 30s`,
...
function adminRootResolveCadaverPartInputV1(input) {
  const clean = String(input || "").trim();
  const index = adminRootParseIntV3(clean, null);
  ...
  if (Number.isFinite(index) && index >= 1 && index <= parts.length) {
    return parts[index - 1];
  }

  const extracted = adminRootExtractCadaverPartFromTextV1(clean);
  return adminRootGetCadaverPartV1(extracted);
}

async function adminRootForceCadaverSpawnNowV1(partKey) {
  if (typeof ensureHolyCorpseEventStateV1 !== "function") {
    return {
      ok: false,
      error: "Sistema de evento do Cadáver Santo não encontrado no index.js."
    };
  }

  const part = adminRootGetCadaverPartV1(partKey);

  if (!part) {
    return {
      ok: false,
      error: `Parte inválida: ${partKey}`
    };
  }
  ...
  if (event.active) {
    const currentPart = adminRootGetCadaverPartV1(event.partKey);

    return {
      ok: false,
      error: [
        `Já existe um evento do Cadáver Santo ativo.`,
        ``,
        `Parte atual: ${currentPart ? `${currentPart.emoji} *${currentPart.name}*` : event.partKey}`,
        `Fase: *${event.phase || "desconhecida"}*`,
        ``,
        `> Aguarde terminar antes de spawnar outro.`
      ].join(adminRootNlV3())
    };
  }
  ...
  if (typeof closeHolyCorpseApproachWindowV1 === "function") {
    setTimeout(() => {
      closeHolyCorpseApproachWindowV1().catch((error) => {
        console.log(`[admin-root] erro ao fechar aproximação forçada do Cadáver Santo: ${error.message}`);
      });
    }, 31 * 1000);
  }
...
async function adminRootScheduleCadaverSpawnV1(message, partKey, delayMs) {
  const part = adminRootGetCadaverPartV1(partKey);
  ...
  if (!part) {
    await adminRootReplyV3(
      ...
      [
        ...
        `> ${partKey}`,
        ``,
        `Use *!admin spawn-cadaver* para abrir o assistente.`
      ].join(adminRootNlV3())
    );
    return;
  }
  ...
  if (safeDelayMs <= 0) {
    const result = await adminRootForceCadaverSpawnNowV1(part.key);
    ...
    await adminRootReplyV3(
      ...
      [
        ...
        `Parte: ${part.emoji} *${part.name}*`,
        ``,
        `> A mensagem pública foi enviada no grupo.`,
        `> Os jogadores têm *30s* para usar *!aproximar*.`
      ].join(adminRootNlV3())
    );
    return;
  }

  const id = ADMIN_ROOT_CADAVER_SPAWN_SEQ_V1++;
  const fireAt = Date.now() + safeDelayMs;

  const timeoutId = setTimeout(async () => {
    ADMIN_ROOT_CADAVER_SPAWN_TIMERS_V1.delete(id);
  ...
  ADMIN_ROOT_CADAVER_SPAWN_TIMERS_V1.set(id, {
    id,
    partKey: part.key,
    partName: part.name,
  ...
  await adminRootReplyV3(
    ...
    [
      ...
      `ID: *${id}*`,
      `Parte: ${part.emoji} *${part.name}*`,
      `Tempo: *${adminRootFormatDelayV1(safeDelayMs)}*`,
      `Horário aproximado:`,
      `> ${new Date(fireAt).toLocaleString("pt-BR")}`,
      ``,
      `> Esse agendamento fica na memória.`,
      `> Se reiniciar o bot antes do horário, ele será perdido.`,
      ``,
      `Cancelar:`,
...
function adminRootParseCadaverSpawnDirectInputV1(parts) {
  const text = String(parts.join(" ") || "").trim();
  ...
  const partKey = adminRootExtractCadaverPartFromTextV1(text);
  const part = partKey ? adminRootGetCadaverPartV1(partKey) : null;
  const delayMs = adminRootParseCadaverDelayMsV1(text);
...
async function adminRootStartCadaverSpawnWizardV1(message, parts) {
  const parsed = adminRootParseCadaverSpawnDirectInputV1(parts || []);

  if (parsed.part && parsed.delayMs !== null) {
    await adminRootScheduleCadaverSpawnV1(message, parsed.part.key, parsed.delayMs);
    return;
  }

  if (parsed.part && parsed.delayMs === null) {
    adminRootSetSessionV3(message, {
      step: "askCadaverSpawnDelay",
      action: {
        type: "spawnCadaver",
        partKey: parsed.part.key
      },
      selectedPart: parsed.part,
      returnMenu: "main"
    });

    await adminRootReplyV3(message, adminRootCadaverSpawnAskDelayTextV1(parsed.part));
    return;
  }

  adminRootSetSessionV3(message, {
    step: "askCadaverSpawnPart",
    action: {
      type: "spawnCadaver"
    },
    returnMenu: "main"
  ...
  await adminRootReplyV3(message, adminRootCadaverSpawnAskPartTextV1());
}

async function adminRootHandleCadaverSpawnSessionV1(message, rawBody, session) {
  const input = String(rawBody || "").trim();
  const normalized = adminRootNormV3(input);
  ...
  if (["voltar", "menu", "0"].includes(normalized)) {
    adminRootClearSessionV3(message);
    await adminRootReplyV3(message, adminRootMainMenuV3());
    return true;
  }

  if (session.step === "askCadaverSpawnPart") {
    const part = adminRootResolveCadaverPartInputV1(input);
    ...
    if (!part) {
      await adminRootReplyV3(
        ...
        [
          `⚠️ *Parte inválida*`,
          ``,
          `Escolha uma parte da lista.`,
          ``,
          adminRootCadaverSpawnAskPartTextV1()
        ].join(adminRootNlV3())
      );
      return true;
    }

    session.step = "askCadaverSpawnDelay";
    session.action.partKey = part.key;
    session.selectedPart = part;
    adminRootSetSessionV3(message, session);

    await adminRootReplyV3(message, adminRootCadaverSpawnAskDelayTextV1(part));
    return true;
  }

  if (session.step === "askCadaverSpawnDelay") {
    const delayMs = adminRootParseCadaverDelayMsV1(input);
    ...
    if (delayMs === null) {
      await adminRootReplyV3(
        ...
        [
          `⚠️ *Tempo inválido*`,
          ``,
          `Digite quando o evento deve aparecer.`,
          ``,
          `Exemplos:`,
          `> agora`,
          `> 1min`,
    ...
    adminRootClearSessionV3(message);
    await adminRootScheduleCadaverSpawnV1(message, session.action.partKey, delayMs);
    return true;
  }
...
async function adminRootListCadaverSchedulesV1(message) {
  const jobs = Array.from(ADMIN_ROOT_CADAVER_SPAWN_TIMERS_V1.values())
  ...
  await adminRootReplyV3(
    ...
    [
      ...
      ``,
      jobs.map((job) => {
        const remaining = Math.max(0, Number(job.fireAt || 0) - Date.now());

        return [
          `${job.id}. ${job.partEmoji} *${job.partName}*`,
          `> Em: ${adminRootFormatDelayV1(remaining)}`,
...
async function adminRootCancelCadaverScheduleV1(message, parts) {
  ...
  const job = ADMIN_ROOT_CADAVER_SPAWN_TIMERS_V1.get(id);
  ...
  await adminRootReplyV3(
    ...
    [
      ...
      ``,
      `ID: *${id}*`,
      `Parte: ${job.partEmoji} *${job.partName}*`
    ].join(adminRootNlV3())
  );
}

// ADMIN_ROOT_CADAVER_SPAWN_V1_END
