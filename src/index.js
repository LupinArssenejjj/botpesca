...
let mandomCoreRestoringV2 = false;

function mandomCoreNlV2() {
  return String.fromCharCode(10);
}
...
function readMandomCoreTimelineV2() {
  try {
    ...
    log("Mandom Core: erro ao ler timeline:", error.message);
    return [];
  }
}
...
function writeMandomCoreTimelineV2(snapshots) {
  try {
    ...
    log("Mandom Core: erro ao salvar timeline:", error.message);
  }
}

function trimMandomCoreTimelineV2(snapshots, now = Date.now()) {
  const minAt = now - MANDOM_CORE_WINDOW_MS_V2 - MANDOM_CORE_KEEP_EXTRA_MS_V2;
...
function pickMandomCoreSnapshotV2(now = Date.now()) {
  ...
  const targetAt = now - MANDOM_CORE_WINDOW_MS_V2;
...
function countMandomCoreHistoryV2(player) {
...
function getMandomCoreEventStatusV2(state) {
...
function buildMandomCoreSummaryV2(beforeState, restoredState, actorId) {
  ...
  for (const id of ids) {
    ...
    const beforeCount = countMandomCoreHistoryV2(beforePlayer);
    const afterCount = countMandomCoreHistoryV2(afterPlayer);
  ...
  const eventChanged = getMandomCoreEventStatusV2(beforeState) !== getMandomCoreEventStatusV2(restoredState);
...
async function activateMandom(state, player) {
  ...
  if (!snapshot) {
    return [
      `⛔ *Mandom falhou.*`,
      ``,
      `Ainda não existe timeline salva.`,
  ...
  restoredActor.stand = { key: "mandom" };
